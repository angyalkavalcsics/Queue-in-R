...
We would like to dedicate the creation of this package to Professor Muhammad El-Taha.
...
print(admit)
# Notice that I intentionally made the data types different
...
# get arrivals in days - or whichever time unit you want
# I will treat this like a simulation where the earliest time begins at time unit 0
...
# fix departure that occurs before arrival
...
In a closed Jackson network, there is no input rate from the outside world. Closed networks have a fixed population that move between the queues but never leave the system. Essentially, this is equivalent to a system where $N$ entities continuously travel inside the network of $K$ nodes. Suppose we have some routing matrix R and we need to solve $vR = v$ such that $\sum v_i = 1$, a condition we force. Recall that the $v_{i}$ are interpreted as the relative throughput through node $i$. 

<br />

We can use the function solverouting() to accomplish this. The only input necessary is the routing matrix and the function returns an error if R is not a square matrix with nonnegative entries so that every row adds up to at most 1. As with open Jackson networks, the routing matrix is assumed to be irreducible and non-absorbing.
...
If you are interested in how this function works, see the following proof. 
...
$$\vec{v} = \vec{v}R$$
or 
$$v_{j} = \sum_{i=1}^{K}v_{i}r_{ij}$$
and require the following
$$\sum_{j}^{K} v_{j} = 1$$
To solve this system of equations, we use techniques from linear algebra. We know that $\vec{v} = \vec{v}R$, hence $v(R-I) = 0$. Now to include the equation $\sum v_{j} = 1$, we add a column of ones to $R - I$. Call this new matrix, $Q$. Then the right hand side of the equation becomes $b = \begin{bmatrix} 0 & 0 & ... & 1 \end{bmatrix}$. That is, $\vec{v} Q = \vec{b}$ and we can now solve for $\vec{v}$. See that
$$\vec{v} Q = \vec{b}$$
$$\vec{v} Q Q^{T} = \vec{b}Q^{T}$$
$$\vec{v} = \vec{b}Q^{T}(Q Q^{T})^{-1}$$
And there you have it. 

<br />

Now, let's get the performance measures for each single-server node in this network using the function cjn.summary(v, N, K, convolution = TRUE). This stands for 'Closed Jackson Network Summary'. The necessary inputs are v, the solution to $vR = v$ such that $\sum v_i = 1$, the given service rates, N the number of customers in the system, K the number of nodes in the network, and a Boolean convolution. When convolution is set to true, the function uses Buzen's algorithm to solve for the summary statistics. Otherwise the function will use Mean Value Analysis. Using the same routing matrix as before let's see an example, first with convolution. 
